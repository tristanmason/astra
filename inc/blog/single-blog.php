...
if ( ! function_exists( 'astra_single_get_post_meta' ) ) {
	...
	function astra_single_get_post_meta( $echo = true ) {
		...
		$post_type_check   = apply_filters( 'astra_single_post_type_meta', array( 'post' ) );

		$output = '';
		if ( is_array( $post_meta ) && ( in_array( $current_post_type, $post_type_check ) || 'attachment' == $current_post_type ) && $enable_meta ) {
