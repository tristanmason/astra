			...
			if ( is_array( $items ) && ! empty( $items ) ) {
				...
				foreach ( $items as $item ) {
					if ( $item['enabled'] ) {
						...
						echo '<a href="' . esc_url( $link ) . '"' . esc_attr( $show_label ? ' aria-label=' . $item['label'] . '' : '' ) . ' ' . ( 'phone' === $item['id'] || 'email' === $item['id'] ? '' : 'target="_blank" rel="noopener noreferrer" ' ) . 'style="--color: ' . esc_attr( $item['color'] ) . '; --background-color: ' . esc_attr( $item['background'] ) . ';" class="ast-builder-social-element ast-' . esc_attr( $item['id'] ) . ' ' . esc_attr( $builder_type ) . '-social-item">';
