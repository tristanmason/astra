...
class AstraColorPickerControl extends Component {
	...
	render() {
		...
		//Backward Compatibity for old palette.
		let oldfinalpaletteColors = [];
		if(astra.customizer.is_old_palette_used){

			disablePalette = false
			const olddefaultColorPalette = [...astColorPalette.colors];

			olddefaultColorPalette.forEach( singleColor => {
				let paletteColors = {};
				Object.assign( paletteColors, { name: count + '_' + singleColor } );
				Object.assign( paletteColors, { color: singleColor } );
				oldfinalpaletteColors.push(paletteColors);
				count ++;
			});
		}
				...
				<div className="color-button-wrap">
					<Button className={ isVisible ? 'astra-color-icon-indicate open' : 'astra-color-icon-indicate' } onClick={ () => { isVisible ? toggleClose() : toggleVisible() } }>
						{ ( 'color' === backgroundType || 'gradient' === backgroundType ) &&						
							<div className="component-color-indicator astra-advanced-color-indicate" style={{backgroundColor:this.props.colorIndicator}}/>
						}
	...
	onChangeComplete( color ) {
		if( this.props.isPaletteUsed){
			this.props.isPaletteUsed(false,'','no');
		}
